  ...
  $(function () {
     ...
     new gridjs.Grid({
      pagination: {
        limit: 5,
        server: {
          url: (prev, page, limit) => `${prev}?limit=${limit}&page=${page * limit}`
        }
      },
      columns: ['Nama Porduk', 'harga', 'Nama Suplier', 'Tanggal pembelian'],
      
      server: {
        url: "<?php echo $this->config->item('URL_API') ?>produk/1",
         headers: new Headers({
        'Authorization': localStorage.getItem("token"), 
        'Content-Type': 'application/x-www-form-urlencoded'
    }), 
        then: data => data.data.map(res => [
              res.name_produk, res.harga,  res.nama_suplier, Date(res.created_at)    
              ]),
