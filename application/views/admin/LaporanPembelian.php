...
<script>
  $(function () {
     ...
     new gridjs.Grid({
      ...
      columns: ['Nama Suplier', 'Nama Produk', 'Nama Kategori', 'Jumlah'],
      
      server: {
        url: "<?php echo $this->config->item('URL_API') ?>laporanPembelianProduk",
         headers: new Headers({
        'Authorization': localStorage.getItem("token"), 
        'Content-Type': 'application/x-www-form-urlencoded'
    }), 
        then: data => data.data.map(res => [
              res.nama_suplier, res.name_produk,  res.nama_kategori, res.count_produk
              ]),
